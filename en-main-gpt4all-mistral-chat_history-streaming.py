...
def create_chain():
    ...
    model = GPT4All(device = 'gpu' , model_name = "alaa_ai_model_mistral_v1.9.gguf" , model_path ='C:/Users/Alaa AI/Python Projects/Ai Models/' , allow_download = False , n_ctx = 2048)
...
if "messages" not in st.session_state:
    st.session_state['model'] = create_chain()
...
with st.session_state['model'].chat_session():
# We take questions/instructions from the chat input to pass to the LLM
    if user_prompt := st.chat_input("Your message here", key="user_input" , on_submit = disable , disabled=st.session_state.disabled):
        ...
        asyncio.run(get_response(st.session_state['model'] , user_prompt , st.empty()))
